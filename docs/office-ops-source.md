...
- **Rep Commission Breakdown adjustments:** "Adjustment between Team & Rep" (the amount Krista types is stored **negated** into fpr_adj_team_to_rep `fldZBFkZCxhmxwNOj`, because the rep-side value is an Airtable formula mirroring the team-side field — comment at `fundraisers.js` line 1035; helper text in the UI: "Positive = team gives rep · Negative = rep gives team") with its label `fld1jNPQUhrowvwK8`; "Misc Adjustment" (rep ↔ SMASH) `fld0iQuhUQDk5L5IY` with its label `fldD8OiuAdjZja6Sq` and comment `fld3vDtAwws1m9EUq` (comment max 500 characters); read-only "Minimum commission adjustment" `fldRk5zGPx6S1Ish0` (Airtable formula; shown only when it has a value — it lifts a negative commission subtotal `fld5BhVUG7Yd4PvT6` back to $0, so rep commission `fldLSmaj4JksmsNUh` is never negative); "Extra boxes ordered" `fldobBrd984o4OLhe` (cookie-dough products only; must be a whole number ≥ 0, error text "Must be 0 or more")
- **Team Profit Breakdown adjustments:** "Misc Adjustment" (team ↔ SMASH) fpr_adj_team_misc `fldQg4pDf7h94NZPZ` with its label `fldw1sM02ULlDzJzj`. Positive = team gets more; Airtable adds it to team profit and subtracts it from the invoice, so SMASH absorbs it. Helper text in the UI: "Positive = team gets more (lowers the invoice). SMASH absorbs it."
- **Misc adjustment rules (both lines):** a typed 0 or blank amount writes null (Airtable never carries a meaningless zero). If the amount is nonzero and the label is blank, Save is blocked with "Add a short description for this adjustment." — but only when that line's amount or label was changed in the current edit session, so older records with unlabeled amounts can still save unrelated edits.
...
`POST /api/echeck/zero-commission` (`echeck.js`). For small fundraisers where rep_commission is exactly $0. Airtable's minimum-commission adjustment already floors the commission at $0, so this flow **never writes an adjustment**. Guards: a positive commission refuses with **"Rep commission is positive — use the normal e-check flow"**; anything else that isn't $0 (within ±$0.005 float tolerance) refuses with "Rep commission is not $0 — check the fundraiser in Airtable before sending". Otherwise it: (1) regenerates the RCR PDF; (2) emails the fresh report to the rep ("…there's no commission payout this time…"); (3) sets rep_paid → true; (4) marks the task Done. **No money moves.** Button label in the wizard: "Send $0 report to rep".
...
- **Staleness:** after generating, a "fingerprint" of the source numbers (MD report attachment ID, manual split values, gross, profit, invoice, commission, upfront card inputs, and every adjustment line with its label — `computeReportFingerprint` in `airtable.js`) is written to `fldrFTjfaCC5SVK1H`. When the detail modal loads, the stored fingerprint is compared to a fresh one; a mismatch shows the amber "may be out of date" warning. The warning is never shown on **Closed Out** or **Cancelled** fundraisers — their attached reports are the historical documents that were sent, and regenerating would replace the originals.
