...
- **Staleness:** after generating, a "fingerprint" of the source numbers (MD report attachment ID, manual split values, gross, profit, invoice, commission, upfront card inputs, and every adjustment line with its label — `computeReportFingerprint` in `airtable.js`) is written to `fldrFTjfaCC5SVK1H`. When the detail modal loads, the stored fingerprint is compared to a fresh one; a mismatch shows the amber "may be out of date" warning.
